...
class FormularioPosGrad(discord.ui.Modal, title='Informações Adicionais — Pós-Graduação'):
    qual_matematica = discord.ui.TextInput(
        label='Em qual área da matemática você tem interesse?',
    ...
    background = discord.ui.TextInput(
        label='Qual é a sua formação/background?',
    ...
    papers_livros = discord.ui.TextInput(
        label='Quais papers/livros de matemática leu recentemente?',
...
class RolesReview(commands.Cog, name='Cargos'):
    ...
    async def setup_cargos(self, ctx):
        ...
        """
        embed = discord.Embed(
            title='🎓 Selecione seu Nível de Matemática',
            description=(
                'Clique no botão abaixo para escolher o cargo que representa seu nível acadêmico.\n\n'
                '📘 **Pré-Universitário** — Ensino médio, vestibulandos, olimpíadas\n'
                '📗 **Graduação** — Cursando ou graduado no ensino superior\n'
                '📕 **Pós-Graduação** — Mestrado, doutorado ou pesquisador\n'
                '*(Pós-Graduação requer preenchimento de formulário e aprovação da equipe)*'
            ),
            color=0x5865F2
        )
        embed.set_footer(text='Você pode trocar seu cargo a qualquer momento.')
